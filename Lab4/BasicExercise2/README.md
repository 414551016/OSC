執行結果：make run
```
lss@DESKTOP-2MCFI1H:~/OscLab4/BasicExercise2$ make run
...
qemu-system-riscv64 -M virt -m 8G -kernel kernel -display none -serial stdio
...
啟用核心定時器的中斷。中斷處理程序應在啟動後每 2 秒打印一次秒數，並將下一個超時設置為 2 秒後。
```
