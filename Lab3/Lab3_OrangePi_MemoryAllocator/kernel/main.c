...
/* ============================================================
 * TA 用 helper：只走 buddy page allocator，不走 chunk pool
 * ============================================================ */
static unsigned int order_from_size(size_t size) {
    size_t pages;
...
/*
 * allocate() - TA 測試用 helper，直接使用 buddy page allocator 進行分配，
 *              不經過 chunk pool。
 *
 * @size: 要求分配的位元組大小。
 *
 * 此 helper 會將請求的大小轉換為頁面 order，分配對應的 buddy 區塊，
 * 然後回傳該區塊的核心虛擬位址。如果請求大小為 0、超出最大 order，
 * 或 page allocator 無法滿足請求，則回傳 NULL。
 */
static void *allocate(size_t size) {
    unsigned int order = order_from_size(size);
    struct page *pg;

    if (order == (unsigned int)-1 || order > MAX_ORDER)
        return NULL;
...
/* ------------------------------------------------------------
 * TA demo：
 * 用純 buddy allocator 跑 TA 測試，避免 chunk pool 造成狀態不回復
 * ------------------------------------------------------------ */
static void ta_test_case(void) {
    printk("\n[TA TEST] initial free_area:\n");
    mm_dump();
    ...
    /*
     * 依你目前初始 free_area 狀態：
     * order0 = 3, order1 = 2, order2 = 1, order3 = 0
     * 這些值來自你的實機 dump。
     */
    enum {
        NUM_BLOCKS_AT_ORDER_0 = 3,
        NUM_BLOCKS_AT_ORDER_1 = 2,
        NUM_BLOCKS_AT_ORDER_2 = 1,
        NUM_BLOCKS_AT_ORDER_3 = 0
    };

    void *ps0[NUM_BLOCKS_AT_ORDER_0 > 0 ? NUM_BLOCKS_AT_ORDER_0 : 1];
    void *ps1[NUM_BLOCKS_AT_ORDER_1 > 0 ? NUM_BLOCKS_AT_ORDER_1 : 1];
    void *ps2[NUM_BLOCKS_AT_ORDER_2 > 0 ? NUM_BLOCKS_AT_ORDER_2 : 1];
    void *ps3[NUM_BLOCKS_AT_ORDER_3 > 0 ? NUM_BLOCKS_AT_ORDER_3 : 1];
...

    {
        ...
        q8  = allocate(4097);
        q9  = allocate(max_block_size + 1); /* 預期為 NULL */
        q10 = allocate(max_block_size);

        printk("q8  = %p\n", q8);
        printk("q9  = %p (expect NULL)\n", q9);
        printk("q10 = %p\n", q10);
